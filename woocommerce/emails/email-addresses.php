...
// --- START CUSTOM CODE to include title ---
// Get the stored title using the correct meta key 'la_title_field'
$customer_title = $order->get_meta( 'la_title_field' ); // Use 'la_title_field' as the key
$billing_first_name = $order->get_billing_first_name();
$billing_last_name = $order->get_billing_last_name();

// If a title exists, try to prepend it to the formatted address by replacing the name.
if ( $customer_title && $billing_first_name && $billing_last_name ) {
    $full_name_without_title = trim( $billing_first_name . ' ' . $billing_last_name );
    $full_name_with_title    = trim( $customer_title . ' ' . $billing_first_name . ' ' . $billing_last_name );

    // Attempt to replace the name in the formatted address string.
    if ( strpos( $address, $full_name_without_title ) !== false ) {
        $address = str_replace( $full_name_without_title, $full_name_with_title, $address );
    } else {
        // Fallback: If the exact full name isn't found, prepend the title and full name
        // to the beginning of the address. This might result in the name appearing twice
        // in rare cases, but ensures the title is present.
        $address = $full_name_with_title . '<br/>' . $address;
    }
}
// --- END CUSTOM CODE ---
