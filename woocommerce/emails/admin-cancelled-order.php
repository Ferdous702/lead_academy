...
// START: MODIFIED CODE
// Get the custom title and the standard full name.
$customer_title = $order->get_meta( 'la_title_field' );
$full_name      = $order->get_formatted_billing_full_name();

// If a title exists, prepend it to the full name.
if ( ! empty( $customer_title ) ) {
	$display_name = $customer_title . ' ' . $full_name;
} else {
	$display_name = $full_name;
}

/* translators: %1$s: Order number. %2$s: Customer full name with title */
...
?>
<p><?php printf( esc_html( $text ), esc_html( $order->get_order_number() ), esc_html( $display_name ) ); ?></p>
<?php
// END: MODIFIED CODE
