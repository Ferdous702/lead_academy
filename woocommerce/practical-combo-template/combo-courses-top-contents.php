...
if ( $product && $product->is_type( 'variable' ) ) :
    ...
    $combo_tabs_data = get_post_meta($product->get_id(), 'combo_course_tabs_group', true);
            ...
            <?php if ( ! empty( $course_types ) ) : ?>
                <div class="la-compact-tabs__container">
                    ...
                    foreach ( $course_types as $course_type ) :
                        $tab_id  = sanitize_title( $course_type ) . '_compact';
                        $fp      = isset($first_prices[$course_type]) ? $first_prices[$course_type] : null;
                        ...
                        >
                            ...
                            <?php echo esc_html( ucwords( str_replace( '-', ' ', $course_type ) ) ); ?>
                        </button>
                        <?php
                        $is_first_tab = false;
                    endforeach;
                ...
                foreach ( $grouped_variations as $course_type => $variations_group ) :
                    $content_id = sanitize_title( $course_type ) . '_compact';
                    $tab_meta = false;
                    if (is_array($combo_tabs_data)) {
                        foreach ($combo_tabs_data as $tab) {
                            if (isset($tab['course_type_slug']) && $tab['course_type_slug'] === $course_type) {
                                $tab_meta = $tab;
                                break;
                            }
                        }
                    ...
                    <div id="<?php echo esc_attr( $content_id ); ?>" class="la-compact-tab-content <?php echo $is_first_content ? 'active' : ''; ?>">
                        ...
                        <div class="la-compact-cards-wrapper">
                            <?php
                            foreach ( $variations_group as $variation ) :
