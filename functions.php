...
// For Face2Face Course top-info-contents.php
// NOTE: This constant is now dynamically managed via the "F2F Products" admin menu
// The Lead Academy F2F Manager plugin will define this constant automatically
// If the plugin is not active, we'll use an empty array as fallback
if (!defined('FACE_2_FACE_PRODUCT_CODES')) {
	$saved_f2f_product_ids = get_option('face_2_face_product_codes_list', array());
	define('FACE_2_FACE_PRODUCT_CODES', is_array($saved_f2f_product_ids) ? $saved_f2f_product_ids : array());
}

define( 'CONSENT_FORM', array(366854,376417,376420,377824,408597,368595,382016,386971,410644,420510,371100,380325, 436829,465279,464447,354284,457511,465279,394220,438477,438480,409404,435450,448459,465279,463897) );

define('PHLEBOTOMY_TRAINING',array(366854,354284,376417,376420,377824,408597,457511,465279,463897));
...
Phlebotomy Training Course in Manchester – Live Class Part 1  457511
Phlebotomy Training Oxford – Part 1 | Hands-On Training With Certification  457585
advanced-phlebotomy-training-in-manchester	464447	
Deep Tissue Massage Training London – Live Class   464492
Phlebotomy Training Cambridge – Part 1   465279
Phlebotomy Training Leeds – Part 1  463897
Reflexology Massage Training Birmingham – Practical Session 465271
*/
...
function la_wp_scripts() {
	...
	if(is_page( 'checkout' )) {
			...
			wp_enqueue_style( 'checkout-sticky-place-order', ASTRA_CHILD_THEME_DIR_URI . '/assets/css/checkout-sticky-place-order.css', array(), time(), 'all' );
			wp_enqueue_script( 'checkout-sticky-place-order', ASTRA_CHILD_THEME_DIR_URI . '/assets/js/checkout-sticky-place-order.js', array('jquery'), time(), true );
	}
	...
	// if(is_product(array(409404))){
	// 	wp_enqueue_style( 'acrylic-nail-extension-course', ASTRA_CHILD_THEME_DIR_URI . '/assets/css/acrylic-nail-extension-course.css', array(), time(), 'all' );
	// }
...
'highfield-qualifications-functional-skills-maths-level-1-past-papers-new'))){
	...
	// For Assignment submission Form video Popup
	if(is_page ( array( 
		452416, // Level 3 Certificate In Assessing Vocational Achievement​ (CAVA) (RQF) Course
		419653, //Assignment - CACHE Award in RQF Health and Social Care
		370323, //Assignment - Education and Training
		431478, //Assignment - CACHE Award in RQF Health and Social Care
		446350, //Assignment – TQUK Level 2 Diploma in Care (RQF)
		397210, //Assignment - Level 3 Diploma in Adult Care – NVQ
		441419 //Assignment - Level 5 Diploma in Teaching
		))
	) {
		wp_enqueue_script( 'la-assignment-form-video-script', ASTRA_CHILD_THEME_DIR_URI . '/assets/js/la-assignment-form-video.js', array('jquery'), time(), true );
		wp_enqueue_style( 'la-assignment-form-video-style', ASTRA_CHILD_THEME_DIR_URI . '/assets/css/la-assignment-form-video.css', array(), time(), 'all' );
	}
...
//Combo Tab Product Specific Scripts
function pti_tab_product_scripts() {
    if (is_product() && get_the_ID() == 371100) {
        wp_enqueue_style('pti-top-tab-style', ASTRA_CHILD_THEME_DIR_URI . '/assets/css/pti-top-tab-style.css');
        wp_enqueue_script('pti-top-tab-js',  ASTRA_CHILD_THEME_DIR_URI . '/assets/js/pti-top-tab.js', array( 'jquery' ) );
    }
}
add_action('wp_enqueue_scripts', 'pti_tab_product_scripts');
...
// Load custom coupon url
require_once ASTRA_CHILD_THEME_DIR . '/inc/coupon-url.php';
...
// Add sticky Place Order bar markup on checkout
add_action('wp_footer', function() {
    if (!function_exists('is_checkout')) { return; }
    if (!is_checkout() || is_order_received_page()) { return; }

    echo '\n<div class="la-sticky-orderbar la-hidden" id="la-sticky-orderbar" aria-hidden="true">'
        . '<div class="la-sticky-orderbar__inner">'
        . '<button type="button" class="la-sticky-orderbar__button" id="la-sticky-scroll-to-payment">Place Order</button>'
        . '</div>'
        . '</div>\n';
});
...
function la_title_field_update_order_meta( $order_id ) {
	...
	if ( ! empty( $_POST['la_profession_field'] ) ) {
		$profession = sanitize_text_field( $_POST['la_profession_field'] );
        $profession = mb_substr( $profession, 0, 45 ); // Limit to 45 characters
        update_post_meta( $order_id, 'la_profession_field', $profession );
	}
}
...
function woocommerce_checkout_adding_payment_title() {
	...
	if ( 
		...
		|| (matched_cart_items( '457409' ) > 0) // TQUK Level 4 Award in the Internal Quality Assurance of Assessment Processes & Practice (RQF)
	 ) {
		?>
		<style>
					...
					function coupon_error_message_change($err, $err_code, $WC_Coupon) {
						switch ( $err_code ) {
							case $WC_Coupon::E_WC_COUPON_INVALID_FILTERED:
								// Check if this is the autumn15 coupon
								if ($WC_Coupon && $WC_Coupon->get_code() === 'autumn15') {
									$err = sprintf(__('The coupon "%s" is applicable only for bundle courses.', 'woocommerce'), $WC_Coupon->get_code());
								} else {
...
add_action( 'template_redirect', 'add_accredited_product_to_cart' );
...
//disable auto p tagging for contact form 7
...
/**
 * Display a custom error message for the 'AUTUMN15' coupon.
 *
 * @param string    $err       The original error message.
 * @param int       $err_code  The numeric error code.
 * @param WC_Coupon $coupon    The coupon object.
 * @return string              The new, custom error message or the original.
 */
add_filter( 'woocommerce_coupon_error', 'custom_autumn15_coupon_message', 10, 3 );

function custom_autumn15_coupon_message( $err, $err_code, $coupon ) {
    
    // The coupon code you want to target.
    $target_coupon_code = 'AUTUMN15';
    
    // Check if the coupon being used is our target.
    if ( $coupon && strtolower( $coupon->get_code() ) === strtolower( $target_coupon_code ) ) {
        
        // Return your custom message for ANY error related to this coupon.
        return 'This "AUTUMN15" coupon is not applicable for this product. Its applicable only for bundle products.';
    }
    
    // For all other coupons, return the original error message.
    return $err;
}